        ...
        {isAdmin ? (
            ...
            <section className="bg-slate-900 rounded-3xl border border-slate-800 overflow-hidden">
              ...
              <table className="w-full text-left text-sm">
                <thead className="text-slate-500 border-b border-slate-800 uppercase text-xs">
                  <tr className="p-4"><th className="p-4">시간</th><th className="p-4">정책</th><th className="p-4">탐지내용</th></tr>
                </thead>
                <tbody>
                  {incidentLogs.map(l => (
          ...
          <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
            <section className="bg-slate-900 p-8 rounded-3xl border border-slate-800 shadow-2xl">
              <textarea className="w-full h-40 bg-slate-950 border border-slate-800 rounded-2xl p-6 mb-4 focus:ring-2 focus:ring-indigo-500 outline-none text-lg text-white" placeholder="가디언이 보호 중입니다. 질문을 입력하세요..." value={input} onChange={e => setInput(e.target.value)} />
              <button onClick={handleAnalyze} className="w-full py-5 bg-indigo-600 rounded-2xl font-black text-xl hover:bg-indigo-500 transition-all text-white">
                {geminiResponse.loading ? "보안 스캔 및 생성 중..." : "안전하게 질문하기"}
              </button>
            </section>
